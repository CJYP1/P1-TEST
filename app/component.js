...
class Component extends DCLogic {
  ...
  selectSubzone(kind,i){
    ...
    if(editable){
      ...
      editHtml=`<div class="sec"><div class="t">Enter data · this sub-division only</div>
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span style="font-size:9.5px;font-weight:800;color:var(--dim);text-transform:uppercase">View</span>
          ...
          <select class="sub-modesel" style="font-size:12px;font-weight:800;padding:5px 9px;border:1px solid var(--line);border-radius:8px;background:var(--panel);color:var(--txt)">${opts}</select>
          <button class="hbtn sub-mnav" data-dir="1" style="padding:4px 10px;font-size:15px;line-height:1">›</button></div>
        <div id="subedit">${this.schedZoneBody(lv,sz)}</div></div>`;
    }
    ...
    if(editable){
      const host=sb.querySelector('#subedit'); if(host)this._bindSchedZone(host,lv,sz);
      const seq=['__total',...this.ACT_MONTHS];
      const msel=sb.querySelector('.sub-modesel'); if(msel)msel.addEventListener('change',()=>{this._schMode=msel.value;this.selectSubzone(kind,i);});
      sb.querySelectorAll('.sub-mnav').forEach(b=>b.addEventListener('click',()=>{const k=seq.indexOf(this._schMode)+(+b.dataset.dir);if(k<0||k>=seq.length)return;this._schMode=seq[k];this.selectSubzone(kind,i);}));
    }
  }
  refreshSubzPanel(){
